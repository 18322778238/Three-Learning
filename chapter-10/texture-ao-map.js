...
const props = {
  ...
  provideGui: (gui, mesh, material) => {
    ...
    textureFolder.add(props, 'normalScaleX', 0, 5, 0.001).name('法线缩放X').onChange(() => {
      material.normalScale.set(props.normalScaleX, props.normalScaleY)
      material.normalMap.needsUpdate = true
    })
    textureFolder.add(props, 'normalScaleY', 0, 5, 0.001).name('法线缩放Y').onChange(() => {
      material.normalScale.set(props.normalScaleX, props.normalScaleY)
      material.normalMap.needsUpdate = true
    })
    textureFolder.add(props, 'aoMapIntensity', 0, 5, 0.001).name('AO贴图强度').onChange((v) => {
