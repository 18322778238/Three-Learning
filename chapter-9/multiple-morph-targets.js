...
bootstrapMeshScene({
  ...
  addControls: (camera, renderer, scene, gui, mesh) => {
    ...
    folder.add(props, 'cubeTarget', 0, 1, 0.01).name('立方体目标').onChange((ev) => (morphTargets[0] = ev))
    folder.add(props, 'coneTarget', 0, 1, 0.01).name('圆锥体目标').onChange((ev) => (morphTargets[1] = ev))
